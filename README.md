# Scaling Umbrella — AI 驱动角色动画生成系统

> AI 理解情绪 → 状态引擎决策 → 动画引擎平滑 → 渲染器输出 → 绿幕视频

面向 **视频制作** 的离线角色动画生成系统。
支持自定义 OC 表情差分（眼开/眼闭 × 嘴开/嘴闭）、AI 音频情绪识别、TTS 接入。
通过 **Streamlit 网页界面** 操作，**图片和音频均只存在于内存，不落盘到项目目录，不会被 git 追踪，关闭页面即消失**。
...
> 💡 图片和音频都不需要提前放到项目文件夹里！每次打开工具时直接在网页上传就好。
> 关闭网页后所有素材自动消失，不会留在项目里，也不会被 git 追踪。
> 音频处理时会短暂写入系统临时目录（`%TEMP%` / `/tmp`，与项目无关），处理完立即删除。
...
> 注意：没有 `assets/` 文件夹！图片和音频全部通过 Streamlit 网页上传，只存在内存（`st.session_state`）中。
> 音频在系统临时目录（`%TEMP%` / `/tmp`）有短暂的处理性落盘，与项目目录完全无关。
...
- [x] 项目结构 + 完整管线
- [x] Streamlit 可视化界面
- [x] 每表情4图上传（眼×嘴组合），纯内存存储
- [x] 音频上传 / TTS 生成，纯内存存储（处理时短暂落盘系统 tmp，用完即删）
- [x] 角色储存功能
- [x] 音频特征提取（RMS + VAD）
- [x] 基础情绪识别（rule 后端）
- [x] 嘴型切换 + 眨眼系统
